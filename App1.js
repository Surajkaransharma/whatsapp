...
async function createClient(userId) {
    ...
    // Check if user-specific session directory exists, else create it
    ...
    const client = new Client({
        authStrategy: new LocalAuth({
            ...
            dataPath: sessionPath, // Save session data to user-specific folder
        }),
    });
    ...
    client.on('qr', async (qr) => {
        ...
        try {
            ...
            console.error('Error generating QR code for user:', err);
        }
    });
    ...
    client.on('disconnected', (reason) => {
        ...
        // Optionally, delete the session folder or handle session cleanup
